  ...
  async function createClaim() {
        ...
        try {
                const payload = {
                          ...
                          estimated_repair_cost: newClaim.estimated_repair_cost
                            ? parseFloat(newClaim.estimated_repair_cost)
                                      : null,
                ...
                if (data.claim) {
                          ...
                          setNewClaim({
                                      ...
                                      builder_email: '', builder_name: userProfile ? (userProfile as any).builder_name || '' : '',
                                      defect_location: '', defect_sub_category: '', estimated_repair_cost: '',
                                      warranty_year: '1', public_story: false
                          })
                          setPendingFiles([])
                          setShowNew(false)
        ...
        try {
                ...
                if (data.success) {
                          ...
                          setClaims(prev => prev.map(c =>
                                      c.id === claim.id
                                                                 ? { ...c, status: 'awaiting_builder', email_thread_address: data.from }
                                        : c
                                                             ))
                }
                                      ...
                                      {/* Category */}
                                                    <div>
                                                                      ...
                                                                      <select value={newClaim.category}
                                                                                            ...
                                                                                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                                        {CATEGORIES.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>option>)}
                                                                      </select>select>
                                                    </div>div>
                                      ...
                                      {/* Severity */}
                                                    <div>
                                                                      ...
                                                                      <select value={newClaim.severity} onChange={e => setNewClaim(p => ({ ...p, severity: e.target.value }))}
                                                                                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                                        {SEVERITIES.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>option>)}
                                                                      </select>select>
                                                    </div>div>
                                      ...
                                      {/* Defect Location */}
                                                    <div>
                                                                      ...
                                                                      <select value={newClaim.defect_location} onChange={e => setNewClaim(p => ({ ...p, defect_location: e.target.value }))}
                                                                        ...
                                                                        {locationOptions.map(l => <option key={l} value={l}>{l}</option>option>)}
                                                                      </select>select>
                                                    </div>div>
                                      ...
                                      {/* Sub-category */}
                                                    <div>
                                                                      ...
                                                                      <select value={newClaim.defect_sub_category} onChange={e => setNewClaim(p => ({ ...p, defect_sub_category: e.target.value }))}
                                                                        ...
                                                                        {subCatOptions.map(s => <option key={s} value={s}>{s}</option>option>)}
                                                                      </select>select>
                                                    </div>div>
                                                                      ...
                                                                      <label className="flex items-center gap-3 cursor-pointer group">
                                                                                          ...
                                                                                          <div>
                                                                                                                ...
                                                                                                                <p className="text-xs text-gray-400 mt-0.5">
                                                                                                                                        Your name and address are never shown. Helps other homeowners see builder patterns.
                                                                                                                  </p>p>
                                                                                            </div>div>
                                                                      </label>label>
                                                    </div>div>
                                                          ...
                                                          <div className="p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleClaim(claim.id)}>
                                                                      ...
                                                                      <span className="text-xs flex items-center gap-0.5 text-green-600">
                                                                                                    <Shield size={10} /> shared
                                                                                                      ...
                                                                                                      {claim.description && <p className="text-sm text-gray-500 mt-1 truncate">{claim.description}</p>p>}
                                                                      ...
                                                                      <span className="text-orange-500 font-medium">
                                                                                                    Est. ${Number((claim as any).estimated_repair_cost).toLocaleString()}
                                                                      </span>span>
                                                                                                                                                      )}

...
const [referralCount, setReferralCount] = useState(0)
    const [referralCardState, setReferralCardState] = useState<'expanded' | 'minimized' | 'dismissed'>('expanded')
...
{/* Share with neighbor card */}
      {userProfile?.referral_code && referralCardState !== 'dismissed' && (
        referralCardState === 'minimized' ? (
          <div className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5 mb-6">
            <div className="flex items-center gap-2 text-sm text-blue-700 font-medium">
              <Share2 size={14} /> Share with your neighbor
            </div>
            <button onClick={expandReferralCard} className="text-xs font-medium text-blue-600 hover:text-blue-800 flex items-center gap-1">
          ...
          <div className="relative mb-6">
            <div className="absolute top-3 right-3 flex items-center gap-1 z-10">
              <button onClick={minimizeReferralCard} aria-label="Minimize" title="Minimize" className="text-white/70 hover:text-white p-1 rounded transition-colors">
                <Minus size={14} />
              </button>
              <button onClick={dismissReferralCard} aria-label="Dismiss" title="Dismiss — you can always find this on your Profile page" className="text-white/70 hover:text-white p-1 rounded transition-colors">
                <X size={14} />
              </button>
            </div>
            <ReferralShareCard

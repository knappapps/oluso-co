...
import { Users, MapPin, Clock, CheckCircle, AlertCircle, Building2, TrendingUp, Search, X, ChevronLeft, ChevronRight, Share2, Facebook, Copy, Check, Mail } from 'lucide-react'
...
const shareUrl = 'https://oluso.co/community'
const shareText = 'New-construction neighbors: homeowners are logging their builder warranty claims in one place and it is eye-opening. ' + totalClaims + ' stories shared so far, ' + resolutionRate + '% resolved, and you can compare how each builder actually handles claims. If you have a warranty issue, documenting it here helps the whole neighborhood hold the builder accountable. Free, no names, no addresses:'
...
{ label: 'Resolution Rate', value: totalClaims > 0 ? resolutionRate + '%' : 'N/A', icon: <TrendingUp size={18} /> },
].map(s => (
...
{/* Share with your neighbors — the neighborhood layer is Oluso's highest-leverage channel.
Buyers of the same builder cluster in the same subdivisions, so a ready-to-paste post
lets one homeowner activate an entire "Residents of [community]" group at once. */}
<div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 md:p-8 mb-12 text-white">
<div className="flex items-center gap-2 mb-2">
<Share2 size={20} />
<h2 className="text-lg font-bold">Share this with your neighbors</h2>
</div>
<p className="text-blue-100 text-sm max-w-2xl mb-5">
Buyers of the same builder live on the same streets and hit the same defects. Drop this into your neighborhood or "Residents of [community]" Facebook or Nextdoor group so more homeowners document their claims — the more stories, the more accountable the builder.
</p>
...
<button
onClick={() => openShare('mailto:?subject=' + encodeURIComponent('Neighbors: builder warranty stories worth seeing') + '&body=' + encodeURIComponent(fullShare))}
className="inline-flex items-center gap-1.5 bg-white text-blue-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors"
>
<Mail size={15} /> Email
</button>
...
>
{shareCopied ? <><Check size={15} /> Copied!</> : <><Copy size={15} /> Copy post</>}
</button>
</div>
</div>
...
<div className="flex flex-wrap gap-2 items-center">
<span className="text-xs text-gray-400 font-medium shrink-0">Severity:</span>
{SEVERITIES.map(sev => (
<button
key={sev}
...
{hasActiveFilters ? (
<>
<p className="text-gray-500 font-medium">No stories match your filters</p>
<button onClick={clearFilters} className="mt-4 text-sm text-blue-600 hover:text-blue-800 font-medium">Clear filters</button>
</>
) : (
<>
...
<Link href={`/community/${nameToSlug(user.community_name || user.city)}`} className="mt-3 pt-2 flex items-center justify-end text-xs text-blue-500 hover:text-blue-700 font-medium">
View neighborhood →
</Link>
)}
</div>
...
<Link href="/dashboard" className="inline-block bg-white text-blue-600 px-5 py-2 rounded-xl text-sm font-semibold hover:bg-blue-50">Go to Dashboard</Link>
</div>
</main>
</div>
)

...
import { ShieldCheck, Mail, User, Building2, MapPin, MessageSquare, CheckCircle, AlertCircle, ArrowRight } from 'lucide-react'
...
export default function ContactPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      const res = await fetch('/.netlify/functions/partner-inquiry', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, email, org, role, area, message }),
      })
      if (!res.ok) {
        ...
        <div className="max-w-2xl mx-auto px-4">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 text-blue-600 font-medium text-sm mb-3">
              <ShieldCheck size={16} /> Partner with Oluso
            </div>
            <h1 className="text-3xl font-bold text-gray-900 mb-3">Get in touch</h1>
            <p className="text-gray-500">
              Are you a home inspector or real estate agent who works with new-construction buyers? Tell us a little about your business and we'll be in touch. No fees, no rev-share.
            </p>
          </div>
          ...
          {success ? (
            <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm text-center">
              ...
              <p className="text-gray-500 text-sm mb-6">
                We received your inquiry and will reach out to <strong>{email}</strong> soon.
              </p>
              <Link href="/partners" className="inline-flex items-center gap-1 text-sm text-blue-600 font-medium hover:text-blue-700">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={org} onChange={e => setOrg(e.target.value)} placeholder="Acme Home Inspections" className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={area} onChange={e => setArea(e.target.value)} placeholder="Salt Lake & Utah County" className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea value={message} onChange={e => setMessage(e.target.value)} rows={4} placeholder="Tell us how you'd like to work together." className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y" />
                  </div>
                </div>

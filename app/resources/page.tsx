...
<div><div className="font-semibold text-amber-700 text-sm">Unlock the full library with Premium</div><div className="text-xs text-amber-600">Get AI tools, executive contacts, and all state-specific guides.</div></div>
...
<div className="grid grid-cols-2 gap-3 mb-6">{[{plan:'Monthly',price:'$9.99/mo'},{plan:'Annual',price:'$79/yr'}].map(p=><div key={p.plan} className="border border-gray-200 rounded-xl p-4 text-center cursor-pointer hover:border-teal-500"><div className="font-semibold text-navy-600">{p.plan}</div><div className="text-amber-500 font-bold">{p.price}</div></div>)}</div>
...
<p className="text-gray-500 text-sm mb-6">You will lose access to Premium features at the end of your billing period. Your claims and data will remain intact.</p>
...
export default function Resources() {
  return <AuthGuard><ResourcesContent /></AuthGuard>
}

      ...
      <div className="max-w-5xl mx-auto px-4 py-8 grid md:grid-cols-3 gap-6">
        ...
        <div className="card md:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setEditing(!editing)} className="flex items-center gap-1 text-amber-500 text-sm font-medium hover:text-amber-600">
              <Edit2 className="w-4 h-4" /> {editing ? 'Save' : 'Edit Profile'}
            </button>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[{label:'Full Name',value:'Sarah Johnson'},{label:'Phone Number',value:'(555) 123-4567'},{label:'Address',value:'1234 Oak Street, Austin, TX 78701'},{label:'Builder Name',value:'Prestige Homes LLC'},{label:'Warranty Start',value:'March 1, 2024'},{label:'Warranty End',value:'March 1, 2025'}].map(f=>(
              <div key={f.label}>
                ...
                {editing
                  ? <input defaultValue={f.value} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                  : <div className="text-sm text-navy-600 font-medium">{f.value}</div>
                }
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <div key={n.key} className="flex items-start justify-between p-4 bg-gray-50 rounded-xl">
                <div>
                  <div className="font-medium text-navy-600 text-sm">{n.label}</div>
